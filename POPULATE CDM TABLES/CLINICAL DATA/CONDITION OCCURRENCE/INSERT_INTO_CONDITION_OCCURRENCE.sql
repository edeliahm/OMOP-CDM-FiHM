...
WITH cte AS (
		...
		FROM BBDD_EmilianoDelia.OMOP_CDM.VISIT_OCCURRENCE V
			...
			/* VISIT IDS FROM THIS TABLE MUST COME FROM THE VISIT_OCCURRENCE TABLE ITSELF */
			 ...
			 --JOIN  [Hosma].DBO.Ingresos I ON V.visit_occurrence_id = I.IdIngreso AND V.visit_concept_id LIKE 9201
		...
		SELECT DISTINCT
			...
			FROM BBDD_EmilianoDelia.OMOP_CDM.VISIT_OCCURRENCE V
				 ...
				 --JOIN [Hosma-Doctor].DBO.AMBULANTES AM ON V.visit_occurrence_id = AM.IdAmbulante AND V.visit_concept_id LIKE 9202
				 JOIN BBDD_EmilianoDelia.OMOP_CDM.PERSON PER ON PER.person_id = V.person_id

 				 JOIN Hosma.DBO.DMedica D ON (V.visit_occurrence_id = D.IdEpisodio AND D.TIPOEPISODIO = 'A')
				 JOIN Hosma.DBO.FichaDMedica10 F ON D.IdDMedica = F.IdDMedica
				 JOIN HOSMA.DBO.CIE10 ON F.IdCie10 = CIE10.IdCIE10
				 JOIN bbdd_emilianodelia.OMOP_CDM.CONCEPT c on c.concept_code = cie10.codigocie10 
